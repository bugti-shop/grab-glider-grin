...
export const OnboardingSlides = ({ onComplete }: Props) => {
  ...
  return (
    ...
    >
      ...
      {/* Back button — bottom left, only shown after first slide */}
      {index > 0 && (
        <div
          className="absolute bottom-0 left-0 z-20 px-4 py-3"
          style={{ paddingBottom: 'calc(var(--safe-bottom, 0px) + 12px)' }}
        >
          <button
            type="button"
            onClick={back}
            className="rounded-full bg-white/85 backdrop-blur-md border border-black/5 px-4 py-2 text-[13px] font-semibold text-black shadow-md active:scale-95 transition"
