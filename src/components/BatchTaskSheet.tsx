...
export const BatchTaskSheet = ({ isOpen, onClose, onAddTasks, sections = [], folders = [] }: BatchTaskSheetProps) => {
  ...
  const gate = (feature: string, run: () => void) => () => { if (!requireProFeature(feature)) return; run(); };
          ...
          <div className="flex gap-3">
            <div className="flex-1 space-y-1.5">
              <label className="text-xs text-muted-foreground flex items-center gap-1.5">
                ...
                {t('tasks.section')}
                {!isPro && <PremiumCrown size={12} />}
              </label>
              <Select
                value={selectedSection || "no-section"}
                onValueChange={(v) => setSelectedSection(v === "no-section" ? "" : v)}
                onOpenChange={(o) => { if (o && !isPro) { requireProFeature('batch_section'); } }}
              >
                <SelectTrigger className="h-9" disabled={!isPro}>
            ...
            <div className="flex-1 space-y-1.5">
              ...
              <Select
                ...
                onValueChange={(v) => setSelectedFolder(v === "no-folder" ? "" : v)}
                onOpenChange={(o) => { if (o && !isPro) { requireProFeature('batch_folder'); } }}
              >
                <SelectTrigger className="h-9" disabled={!isPro}>
          ...
          <div className="flex gap-3">
            <div className="flex-1 space-y-1.5">
              ...
              <Select
                ...
                onOpenChange={(o) => { if (o && !isPro) { requireProFeature('batch_priority'); } }}
              >
                <SelectTrigger className="h-9" disabled={!isPro}>
                  <SelectValue>
            ...
            <div className="flex-1 space-y-1.5">
              ...
              <Popover onOpenChange={(o) => { if (o && !isPro) { requireProFeature('batch_due_date'); } }}>
