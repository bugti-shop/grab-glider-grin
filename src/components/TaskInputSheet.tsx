...
import { MentionDescriptionEditor } from './richtext/MentionDescriptionEditor';
...
export const TaskInputSheet = ({ isOpen, onClose: rawOnClose, onAddTask, folders, selectedFolderId, onCreateFolder, onEnsureFolder, onEnsureSection, sections = [], selectedSectionId, defaultDate, preventBackdropClose = false, autoOpenScanner = false }: TaskInputSheetProps) => {
  ...
  useEffect(() => {
    setSectionId(selectedSectionId ?? undefined);
  ...
  const handleSend = (e?: React.MouseEvent) => {
    ...
    let finalFolderId = folderId ?? selectedFolderId ?? undefined;
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Free plan: capacity gate (1 attachment per calendar day)
    ...
    for (const file of Array.from(files)) {
      ...
      reader.onloadend = async () => {
        ...
        added += 1;
        incrementAttachmentsAddedToday(1);
      };
      reader.readAsDataURL(file);
    }
          ...
          {descriptionPreview && (
            <div className="px-4 py-2 bg-cyan-50 dark:bg-cyan-950/20 rounded-lg flex items-center gap-2 mb-4">
              ...
              <span className="text-sm text-cyan-700 dark:text-cyan-300 font-medium truncate flex-1">
                {descriptionPreview.length > 50 ? descriptionPreview.substring(0, 50) + '...' : descriptionPreview}
              </span>
              <button
                onClick={() => {
                  setDescription('');
