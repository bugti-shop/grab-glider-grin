            ...
            {showNotificationsExpanded && (
              <div className="bg-muted/30">
                ...
                <div className="px-4 py-3 border-b border-border/50 space-y-2">
                  <div>
                    <span className="text-foreground text-sm block">{t('settings.habitReminders', 'Habit Reminders')}</span>
                    <span className="text-xs text-muted-foreground">
                      {t('settings.habitRemindersDesc', 'Enable notifications for every habit reminder')}
                    </span>
                  </div>
                    ...
                    <button
                      onClick={async () => {
                        ...
                        if (Capacitor.isNativePlatform()) {
                          ...
                          if (res.display === 'granted') toast.success(t('settings.notifPermGranted', 'Notifications enabled'));
                          else toast.error(t('settings.notifPermDenied', 'Permission denied. Enable in system settings.'));
                        } else {
                          const { requestNotificationPermission } = await import('@/utils/webNotifications');
                          const r = await requestNotificationPermission();
                          if (r === 'granted') toast.success(t('settings.notifPermGranted', 'Notifications enabled'));
                          else if (r === 'unsupported') toast.error(t('settings.notifUnsupported', 'Notifications not supported on this browser'));
                          else toast.error(t('settings.notifPermDenied', 'Permission denied. Enable in browser settings.'));
                        }
                      }}
                      className="flex-1 px-3 py-2 text-xs font-medium rounded-md bg-primary text-primary-foreground hover:bg-primary/90"
                    >
                      {t('settings.enableNotifications', 'Enable notifications')}
                    </button>
                    <button
                      onClick={async () => {
                        const { testHabitReminder } = await import('@/utils/habitReminders');
                        await testHabitReminder('Test habit reminder ✨', '🔔', 2000);
                        toast.success(t('settings.testNotifSent', 'Test notification in ~2s'));
                      }}
                      className="flex-1 px-3 py-2 text-xs font-medium rounded-md border border-border hover:bg-muted"
                    >
                      {t('settings.sendTest', 'Send test')}
                    </button>
                  </div>
                </div>
              </div>
