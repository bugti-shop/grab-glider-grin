...
const Today = () => {
  ...
  const { softRequireCreate, canCreateWithinSoftLimit, openPaywall } = useSubscription();
  ...
  // openFirstTaskForTour listener is registered further below, after
  // handleAddTask is available from useTodayActions.
  ...
  // (Default "My First Task" seeder removed — users start with an empty list.)
                          ...
                          <Draggable key={folder.id} draggableId={`folder-chip-${folder.id}`} index={index}>
                            {(dragProvided, snapshot) => (
                              <button
                                ...
                                className={cn("flex items-center gap-2 px-4 py-2 rounded-full transition-all whitespace-nowrap flex-shrink-0 text-primary-foreground", isSelected ? "ring-2 ring-inset ring-white/70" : "hover:opacity-90", snapshot.isDragging && "shadow-lg opacity-90")}
                                style={{
